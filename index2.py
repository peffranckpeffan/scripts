...
if goFoward:
	...
	if(stage == 'us' and ifExec=="exec"):
		try:
			phaseUs=str(sys.argv[3])
		except:
			print("It's necessary to inform de phase of the PMF simulations (0 if the first time, 1 for de second and so on).")
			goFoward=False
...
if stage=="analysis":
	...
	for x in range(len(folders)):
		for i in range(1,num_fold):
			window=str(i).zfill(2)
			window2=str(i-1).zfill(2)
			folder_copy = folders[x]+window
			folder_for = folders[x]+window2
